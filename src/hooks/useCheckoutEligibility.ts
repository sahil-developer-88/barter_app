...
export function useCheckoutEligibility(
  ...
  const calculate = async () => {
    if (!posTransaction || !user) {
      console.log('⏭️ Skipping calculation - no transaction or user');
      return;
    }
    ...
    try {
      console.log('\n🚀 useCheckoutEligibility - Starting calculation...');
      console.log(`📦 Transaction ID: ${posTransaction.id}`);
      console.log(`👤 Merchant ID: ${user.id}`);
      console.log(`💳 Items: ${posTransaction.items.length}`);
